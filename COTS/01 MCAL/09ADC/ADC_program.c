...
//7. Set the Respective GPIO PINs in the Analog Mode
	GPIOA_CRL &= ~(0xf<<4);  // analog mode for PA 1
	GPIOA_CRL &= ~(0xf<<16); // analog mode for PA 4
...
//	DMA1_Channel7->CCR |= (1<<4);   // Read From Memory
	DMA1->Channel[0].CCR &= ~(1<<4);   // Read From Peripheral

	// 2. Enable the circular mode (CIRC)
	DMA1->Channel[0].CCR |= 1<<5;

	// 3. Enable the Memory Increment (MINC)
	DMA1->Channel[0].CCR |= 1<<7;

	// 4. Set the Peripheral data size (PSIZE)
	DMA1->Channel[0].CCR |= (1<<8);  // 01 : 8 Bit Data

	// 5. Set the Memory data size (MSIZE)
	DMA1->Channel[0].CCR |= (1<<10);  // 01 : 8 Bit Data
}


void DMA_Config (u32 srcAdd, u32 destAdd, u16 size)
{
	...
	************************************************/
	...
	DMA1->Channel[0].CNDTR = size;   // Set the size of the transfer

	DMA1->Channel[0].CPAR = srcAdd;  // Source address is peripheral address

	DMA1->Channel[0].CMAR = destAdd;  // Destination Address is memory address

	// Enable the DMA Stream
	DMA1->Channel[0].CCR |= (1<<0);  // EN =1
}
...
int main(void)
{
	...
	DMA_Config ((u32 ) &ADC1->DR, (u32) RxData, 3);
